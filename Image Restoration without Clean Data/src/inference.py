...
NOISE_TYPE = 'text'
NOISE_PARAM = (NOISE_TYPE, 0.25)

#NOISE_TYPE = 'poisson'
#NOISE_PARAM = (NOISE_TYPE, 125)

test_ds = NoisyDataset(test_dir, crop_size=128, train_noise_model=NOISE_PARAM, clean_targ=True)
# gaussian poisson multiplicative_bernoulli text

x1, _, y1 = test_ds.__getitem__(-1)
x1.show()   #noisey
x1.save("x_text_bsds300.jpg")
y1.show()   #clean
y1.save("y_text_bsds300.jpg")

img_tensor_x = image.img_to_array(x1)
img_tensor_x = np.expand_dims(img_tensor_x, axis=0)
img_tensor_x /= 255
print(img_tensor_x.shape)

img_tensor_y = image.img_to_array(y1)
img_tensor_y = np.expand_dims(img_tensor_y, axis=0)
img_tensor_y /= 255
print(img_tensor_y.shape)

# gaussian_bsds300_model.h5
# gaussian_kodak_model.h5
# multiplicative_bernoulli_bsds300_model.h5
# multiplicative_bernoulli_kodak_model.h5
# poisson_bsds300_model.h5
# poisson_kodak_model.h5
# text_bsds300_model.h5
# text_kodak_model.h5

model = load_model('saved_models/text_bsds300_model.h5')
y_hat = model.predict(img_tensor_x)
print(y_hat.shape)

y_hat = y_hat[0, :, :, :]
print(y_hat.shape)
plt.imshow((y_hat * 255).astype(np.uint8))
plt.savefig('y_hat_text_bsds300.jpg')
plt.show()
